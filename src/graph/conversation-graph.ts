 ...
 * 
 * Simplified flow for Renatinhu's Cars:
 * START → Greeting → Discovery (name, budget, usage, preferences) → Search → Recommendation → END
 * 
 ...
 * Requirements: 4.5 - Budget and at least one preference
 */
 ...
 * Requirements: 4.5 - Transition to recommendation when profile is complete
 */
function routeNext(state: ConversationState): string {
  ...
  switch (currentNode) {
    ...
    case 'discovery':
      // Check if profile is complete (budget + at least one preference)
      if (state.quiz.isComplete || isProfileComplete(state.profile)) {
        return 'search';
      }
      return 'discovery'; // Stay in discovery until complete

    case 'quiz':
      // Legacy support - redirect to discovery
  ...
  async invoke(input: {
    ...
    try {
      switch (state.graph.currentNode) {
        ...
        case 'discovery':
          update = await discoveryNode(state);
          break;

        case 'quiz':
          // Legacy support - redirect to discovery node
          update = await discoveryNode(state);
          break;
